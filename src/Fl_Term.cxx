...
//
// Copyright 2022-2023 by Raphael Kim.
...
int Fl_Term::connect(HOST *newhost, const char **preply )
{
    ...
    host->connect();
    if ( preply!=NULL ) 
    {   //waitfor prompt if called from script
...
// parse text received from host 
void Fl_Term::puts( const char *buf, int len )
{
    if ( len==0 ) 
    {//Connected, send term size
...
Fl_Term::Fl_Term(int X,int Y,int W,int H,const char *L) 
  : Fl_Widget(X,Y,W,H,L)
{
    bEcho = false;
    bScrollbar = false;
...
int Fl_Term::handle(int e)
{
    switch (e) 
    {
        ...
        case FL_DRAG:
            if ( Fl::event_button()==FL_LEFT_MOUSE ) 
            {
                ...
                else 
                {
                    ...
                    while ( (buff[sel_right]&0xc0)==0x80 ) 
                    {
                        sel_right++;
                    }
                }
        ...
        case FL_PASTE:
            if ( bDND ) 
            {   //drag and drop run as script
                run_script(Fl::event_text());
            }
            else 
            {   //or paste to send to host
        ...
        case FL_KEYDOWN:
            ...
            switch (key) 
            {
                ...
                case FL_Up:   
                    host->write(bAppCursor?"\033OA":"\033[A",3); 
                    break;
                case FL_Down:
                    host->write(bAppCursor?"\033OB":"\033[B",3); 
                    break;
                case FL_Right:
                    host->write(bAppCursor?"\033OC":"\033[C",3); 
                    break;
                case FL_Left: 
                    host->write(bAppCursor?"\033OD":"\033[D",3); 
                    break;
...
void Fl_Term::next_line()
{
    ...
    if ( cursor_x>buff_size-1024 || cursor_y>line_size-3 ) 
    {
        Fl::lock();
        if ( line_size<65536 ) 
        {   //double buffer size till 64k lines
            char *old_buff = buff;
            char *old_attr = attr;
            int *old_line = line;
            ...
            {   //clear buffer if failed to double
        ...
        {   //erase half of buffer at 64k lines
...
void Fl_Term::append( const char *newtext, int len )
{
    ...
    // only one thread should be appended to buffer at a time.
    append_mtx.lock();

    if ( fpLogFile!=NULL ) 
        fwrite( newtext, 1, len, fpLogFile );
    ...
    if ( !bPrompt && cursor_x>iPrompt ) 
    {
        ...
        if ( strncmp(p, sPrompt, iPrompt)==0 ) 
            bPrompt=true;
    }
...
void Fl_Term::termsize(int cols, int rows)
{
    if ( size_x!=cols || size_y!=rows ) 
    {
        ...
        screen_clear(2);
        // this callback will trigger to resize window -
        do_callback(this, (void *)sTitle);
    }
}
                            ...
                            if ( ESC_code[1]=='?' ) 
                            {
                                switch( atoi(ESC_code+2) ) 
                                {
                                    ...
                                    case 2004: 
                                        bBracket = true; 
                                        break;
                                    case 1049: /// ?1049h alternate screen
                                        bAltScreen = true;
                                        screen_clear(2);
                                        break;
                                }
                            ...
                            if ( ESC_code[1]=='?' ) 
                            {
                                switch( atoi(ESC_code+2) ) 
                                {
                                    ...
                                    case 2004: 
                                        bBracket= false; 
                                        break;
                                    case 1049: /// ?1049l alternate screen
                                        bAltScreen= false;
                                        cursor_y = screen_y;
                                        cursor_x = line[cursor_y];
                        ...
                        case 'm': //text style, color attributes
                            {
                                ...
                                while ( p!=NULL ) 
                                {
                                    ...
                                    switch ( m0/10 ) 
                                    {
                                        ...
                                        case 3: 
                                                if ( m0==39 ) m0 = 7;//default foreground
                                                c_attr = (c_attr&0xf8)+m0%10;
                                                break;

                                        case 4: 
                                                if ( m0==49 ) m0 = 0;//default background
